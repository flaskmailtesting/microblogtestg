...
from megatutApp.models import User, Post, Message, Notification
from megatutApp.translate import translate
from megatutApp.main.forms import EditProfileForm, PostForm, SearchForm, MessageForm
from megatutApp.main import bp
...
@login_required
def edit_profile():
	eform = EditProfileForm(current_user.username)
	if eform.validate_on_submit():
		current_user.username = eform.username.data
		current_user.about_me = eform.about_me.data
		db.session.commit()
		flash(_("Your changes have been changed."))
		return redirect(url_for("main.edit_profile"))
	elif request.method == "GET":
		eform.username.data = current_user.username
		eform.about_me.data = current_user.about_me
	return render_template("edit_profile.html", title=_("Edit Profile"), form=eform)
...
def send_message(recipient):
	...
	if mform.validate_on_submit():
		msg = Message(author=current_user, recipient=user, body=mform.message.data)
		db.session.add(msg)
		user.add_notification("unread_message_count", user.new_messages())
		db.session.commit()
		flash(_("Your message has been sent."))
	return render_template("send_message.html", title=_("Send Message"), form=mform, recipient=recipient)
